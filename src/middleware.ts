...
export function middleware(req: NextRequest) {
    ...
    // If we don't have a canonical host, do nothing
    ...
    // Allow ANY other subdomain of the apex (your sandbox/tab subdomains)
    // Example: tab-123.ronsmusicstore.com → allow
    if (host.endsWith(`.${APEX_HOST}`) && host !== `www.${APEX_HOST}`) {
        return NextResponse.next();
    }
